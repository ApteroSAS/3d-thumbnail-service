...
export default class BlenderEngine {
    ...
    async script2glb(script: string, outputFile: string = "/out/obj.glb"): Promise<{ filepath: string, folder: string }> {
        const folder = await this.createTmpFolder();
        script+="\nfilepath = \"/media"+outputFile+"\"\n" +
            "bpy.ops.export_scene.gltf(filepath=filepath, export_format='GLB')"
        Fs.writeFileSync(folder + '/script.py', script);
        try{
            await this.execBlenderDocker(folder,false,"3.2.0-1.0.0");
        }catch (e) {
            console.error(e);
            //try{
                await this.execBlenderDocker(folder,false,"2.82-1.0.0");
            /*}catch (e) {
                console.error(e);
                await this.execBlenderDocker(folder,false,"2.79-latest");
            }*/
        }
        return {
            filepath: folder + outputFile,
            folder: folder
        }
    }

    async description2glbAI(description: string): Promise<{ filepath: string, folder: string }> {
        let script = await this.ai.textToPythonBlenderScript(description);
        return this.script2glb(script);
    }
    ...
    private async execBlenderDocker(folder, mapEntireMedia = true,tag:string=null) {
        ...
        await new Promise((resolve, reject) => {
            //useage a 3d glb file must be present in the media folder "3dfile.glb"
            const image = "registry.aptero.co/docker-blender:" + (tag || config.TAG || "latest")
            let volumeMap = "";
            if (mapEntireMedia) {
                volumeMap = " -v " + folder + "/:/media/ "
            } else {
                volumeMap = " -v " + folder + "/out/:/media/out/ -v "+folder+"/script.py:/media/script.py "
            }
            let cmd = "docker run --rm -e DISPLAY_WIDTH=" + (config["DISPLAY_WIDTH"] || 1024) + " -e DISPLAY_HEIGHT=" + (config["DISPLAY_HEIGHT"] || 768) + volumeMap + image;
            console.log(cmd);
            exec(cmd, (error, stdout, stderr) => {
                if (error) {
                    console.log(`error: ${error.message}`);
